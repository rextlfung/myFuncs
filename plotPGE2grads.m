function W = plotPGE2grads(ceq, sys, varargin)
% function W = plotPGE2grads(ceq, sys, varargin)
%
...
linkaxes([ax{1} ax{2} ax{3}], 'x');  % common zoom setting (along time axis) for all tiles
...
if arg.interpolate
    msg = sprintf('%sGradient waveforms interpolated to 4us.', msg);
else
    msg = sprintf('%sWaveform samples/corner points are shown as defined in the original Pulseq file.', msg);
end
title(msg, 'FontSize', 24);
...
function sub_plotboundary(T, tp)
    ...
    for m = 1:length(T)
        if strcmp(tp, 'block')
            xline(T(m), ':', 'linewidth', 1.2, 'color', 'k');
        else
...
function sub_addSegmentInstanceToPlot(tic, S, showBlocks, sysGE, yLim, nSubPlots)
    ...
    for g = {'gx','gy','gz'}
        ...
        S.(g{1}).signal(isnan(S.(g{1}).signal)) = 0;

        subplot(nSubPlots,1,sp); hold on;
        plot(1e3*(tic + S.(g{1}).t), S.(g{1}).signal, [cols(sp) '.-']);
        ylabel({g{1}, 'G/cm'}, 'Rotation', 0, 'FontSize', 16);
        yl = 1.05 * yLim.(g{1});
        ylim(yl * [-1 1]);
        if showBlocks
